  ...
  return (
    <div className="rounded-2xl border border-border overflow-hidden bg-card shadow-sm" dir="rtl">
      ...
      <div className={cn('px-4 py-3 flex items-center gap-3 border-b border-border/50',
        isManual ? 'bg-navy/5' : 'bg-[#FC4C02]/5')}>
        ...
        {pctOfPlanned != null && (
          <span className={cn('text-xs font-bold px-2 py-1 rounded-full flex-shrink-0',
            pctOfPlanned >= 95 ? 'bg-pine/15 text-pine' : 'bg-ochre/15 text-ochre-deep')}>
      ...
      <div className="px-4 py-3 grid grid-cols-2 gap-x-6 gap-y-3">
        {duration && (
          <div>
            ...
            <p className="text-xl font-black text-navy">{duration}</p>
          </div>
        )}
        ...
        {log.actualPace && (
          <div>
            ...
            <p className="text-xl font-black text-navy" dir="ltr">{log.actualPace}</p>
          </div>
        )}
        ...
        {log.elevationGain != null && log.elevationGain > 0 && (
          <div>
            ...
            <p className="text-xl font-black text-pine">+{log.elevationGain}<span className="text-sm font-semibold">m</span></p>
          </div>
        )}
      </div>
          ...
          <div className="divide-y divide-border/20">
            {splits.map((split: any, i: number) => (
              <div key={i}
                ...
                <span className="font-bold text-navy">{split.pace || '—'}</span>
          ...
          {log.effort != null && (
            <div className="flex items-center gap-2">
              <span className={cn('w-2.5 h-2.5 rounded-full flex-shrink-0',
                log.effort <= 4 ? 'bg-pine' :
                log.effort <= 6 ? 'bg-ochre' :
                log.effort <= 7 ? 'bg-orange-400' : 'bg-rust')} />
              <p className="text-sm font-bold text-navy">מאמץ {log.effort}/10</p>
            </div>
          )}
          {log.comment && (
            <p className="text-sm text-foreground/80 italic leading-snug">"{log.comment}"</p>
          )}
        </div>
      )}
    </div>
