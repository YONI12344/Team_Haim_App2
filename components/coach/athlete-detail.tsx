    ...
    <div className="space-y-6">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          // Get athlete stravaId
                          const userSnap = await import('firebase/firestore').then(m => m.getDoc(m.doc(db, 'users', athleteId)))
                          const stravaId = userSnap.data()?.stravaId
                          if (stravaId) {
                            // Call Strava deauthorize API to free up the slot
                            const connSnap = await import('firebase/firestore').then(m => m.getDoc(m.doc(db, 'strava_connections', `strava_${stravaId}`)))
                            if (connSnap.exists()) {
                              const accessToken = connSnap.data()?.accessToken
                              if (accessToken) {
                            ...
                            await deleteDoc(doc(db, 'strava_connections', `strava_${stravaId}`))
                          }
                          // Remove stravaId from user doc
                          await updateDoc(doc(db, 'users', athleteId), { stravaId: null, stravaConnected: false })
                          // Delete all strava logs
                          const logsSnap = await getDocs(query(collection(db, 'logs'), where('athleteId', '==', athleteId), where('source', '==', 'strava')))
                          for (const d of logsSnap.docs) await deleteDoc(d.ref)
                          toast.success('Strava נותק בהצלחה ✓')
                        } catch(e) { console.error(e); toast.error('שגיאה בניתוק') }
                      }}
                    >
                      🔌 נתק Strava
                    </Button>
      ...
      <Tabs defaultValue="schedule" className="space-y-6">
        <TabsList className="flex flex-wrap gap-1 h-auto">
          ...
          <TabsTrigger value="documents">📄 מסמכים</TabsTrigger>        </TabsList>
