...
import { getJourney, stageDisplayName } from '@/lib/journey'
import { stepsFromPhysiologySummary } from '@/lib/physiology'
import type { JourneyDoc, JourneyStage } from '@/lib/types'
import type { DayKey } from '@/lib/ai-coach-brain/backstops'
import { DAY_INDEX } from '@/lib/ai-coach-brain/backstops'
import {
  AI_JOURNEY_ID,
  DAY_ORDER,
  GENERATION_TARGET_STAGES,
  runSeasonPipeline,
...
const UI = {
  en: {
    cardTitle: 'Season generator',
    cardDesc: "Everything the AI coach uses to build this athlete's season. Generating writes the phases and day-by-day workouts from the Norwegian Method brain straight into the planner, with lab-derived pace/HR targets. The athlete sees the first 2 weeks; the rest reveals every Saturday.",
    knowsAbout: (name: string) => `Everything Assistant Coach AI knows about ${name} — edit here before generating if it changed`,
    ...
    labTestYes: '✓ Lab step test on file — Assistant Coach AI will compute exact lab-derived pace/HR targets for every quality session.',
    labTestNoBase: 'No lab step test on file. Not required — Assistant Coach AI will fall back to HR% / talk test / RPE (per the brain\'s intensity_triangulation)',
    ...
    coachNotesLabel: 'Coach notes for Assistant Coach AI (private, never shown to athlete)',
    save: 'Save',
    ...
    clearingPrevious: 'Clearing previous Assistant Coach AI-generated season...',
    ...
    setGoalRaceFirst: "Set a Goal Race Date for this athlete first — go to their profile page → Profile tab → Edit Profile → Goal Race Date — then generate the season plan.",
    ...
    seasonWritten: (n: number) => `Season plan written: ${n} workouts`,
    generateFailed: 'Failed to generate Assistant Coach AI plan',
  ...
  he: {
    cardTitle: 'מחולל העונה',
    cardDesc: 'כל מה שמאמן ה-AI משתמש בו כדי לבנות את העונה של הספורטאי. היצירה כותבת את השלבים והאימונים יום-אחר-יום מהמוח של השיטה הנורבגית ישירות ללוח האימונים, עם יעדי קצב/דופק מבוססי מעבדה. הספורטאי רואה את השבועיים הראשונים; השאר נחשף כל שבת.',
    knowsAbout: (name: string) => `כל מה שעוזר המאמן ה-AI יודע על ${name} — ניתן לערוך כאן לפני היצירה אם משהו השתנה`,
    ...
    labTestYes: '✓ בדיקת מדרגות מעבדה קיימת — עוזר המאמן ה-AI יחשב יעדי קצב/דופק מדויקים מבוססי מעבדה לכל אימון איכות.',
    labTestNoBase: 'אין בדיקת מדרגות מעבדה. לא חובה — עוזר המאמן ה-AI ייעזר בדופק%/מבחן שיחה/RPE (לפי המוח, intensity_triangulation)',
    ...
    coachNotesLabel: 'הערות מאמן לעוזר המאמן ה-AI (פרטי, לא מוצג לספורטאי)',
    save: 'שמור',
    ...
    clearingPrevious: 'מנקה עונה קודמת שנוצרה על ידי עוזר המאמן...',
    ...
    setGoalRaceFirst: 'קבע/י תאריך מירוץ יעד לספורטאי קודם — לך/י לעמוד הפרופיל שלו ← לשונית פרופיל ← עריכת פרופיל ← תאריך מירוץ יעד — ואז צור/י את תוכנית העונה.',
    ...
    seasonWritten: (n: number) => `תוכנית העונה נכתבה: ${n} אימונים`,
    generateFailed: 'יצירת תוכנית עוזר המאמן נכשלה',
...
/**
 * Plan settings for the AI coach (opened from the chat panel): everything
 * the season generator reads — availability, long-run day, goal race, PRs,
 * recurring sessions, special events, cutback weeks, coach notes — plus a
 * direct "Generate" button. Generation itself is the shared pipeline in
 * lib/ai-coach/season-pipeline.ts (18-chapter brain), the same code the
 * chat agent's generate_season_plan tool runs.
 */
...
export function AiPlanSettings({ athleteId }: { athleteId: string }) {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    load()
    getJourney(athleteId, AI_JOURNEY_ID).then((j) => { if (!cancelled) setJourneyPreview(j) })
  ...
  const generate = async () => {
    ...
    setProgress(null)
    try {
      // Persist whatever the coach set/adjusted above — the pipeline reads
      // everything (schedule, cutback, recurring activities...) from the
      // profile, exactly as the chat agent's generate_season_plan does.
      // Only 'workout' days count as training days ('rest' is a deliberate day off).
      ...
      const result = await runSeasonPipeline({
        ...
        onJourney: setJourneyPreview,
      })
      if (!result.ok) {
        if (result.alreadyComplete) toast.success(result.error)
      ...
      setLastSummary(`${result.summary}\n\n${result.firstBlockSummary ?? ''}`)
      toast.success(t.seasonWritten(result.written))
    } catch (e) {
      console.error('AI season generation failed:', e)
