...
11. DOUBLE THRESHOLD DAYS (workout_mechanics_and_first_principles.double_threshold) — ONLY when the athlete is on the ambitious_elite track (daysPerWeek >= 6 and experienceLevel advanced/professional, or the current stage explicitly calls for it): pick at most 2 non-consecutive days per week (e.g. Tue/Thu, per training_structures_by_level.ambitious_elite.standard_week) and emit TWO separate workout objects that share the exact same "date" — one with session "am" (longer repeats, lower lactate ~2.0-2.5 mmol/L) and one with session "pm" (shorter repeats, higher lactate ~3.0-3.5 mmol/L). VARY THE REP FORMAT — don't default to the same "5x6min AM / 10x1000m PM" every single double-threshold day across the season; rotate through the options in double_threshold.am_session/pm_session (6min/8min/10min reps, 2km/3km reps, mile/1km reps for AM; 1000m/400m/mile/1km reps for PM) so back-to-back double-threshold sessions feel genuinely different, not copy-pasted. Follow double_threshold.progression_within_a_season: base phase leans on the longer AM formats (3km/2km/8-10min), build/peak sharpens the AM session shorter and faster toward race-pace-adjacent effort — EXCEPT for a marathon-distance athlete, who should keep some longer/marathon-pace-adjacent AM work even at peak rather than converging on short fast reps. Never do double-threshold for recreational or intermediate tracks — they get exactly one workout object per date, session "other".
12. PER-TYPE CONTENT REQUIREMENTS — every workout of every type must be fully specified, not just intervals/tempo:
    - "easy" / "recovery": distance and/or duration filled, bakkenLactateMin/Max set per rule 6 (1.0-1.2), description states the effort explicitly ("easy, conversational, under 70% HRmax — when in doubt, run slower, not faster" per foundational_principles.principle_4_easy_is_sacred), sets empty — UNLESS this is the absolute-beginner run/walk case (rule 5c), which needs sets[] populated instead.
    - "long_run": distance and duration filled. If athlete_context.longRunMinutes is set, no long run this season should exceed it — pick duration at or below that cap, distance follows from pace, not the other way around. Most long runs stay purely easy (sets empty) — that's the default. But especially in build/peak phase (and always for a marathon athlete per distance_specific_adjustments.marathon.long_runs), some long runs should be "quality long runs" with alternating threshold/easy segments instead — e.g. 3km @ T1 sub-threshold, then 1km easy, repeated 2-4 times, or a single marathon-pace tail segment (e.g. "final 8-10km at goal marathon pace"). THIS IS CONTINUOUS RUNNING THE WHOLE TIME — there is no stop, no walk break, no rest between segments; the "easy" portions are easy-paced RUNNING that serves as the recovery, exactly like a real progressive/broken-up long run. Do not put anything in restBetweenReps for this structure (that implies a stop) — it's purely a pace change between consecutive intervals[] segments. Build it using intervals[] exactly like an alternating fartlek (rule 12's fartlek guidance) — one set with reps = number of threshold/easy cycles, intervals[] listing each segment in order (distanceMeters + notes stating the pace/effort, e.g. "T1 sub-threshold" vs "easy, no stopping"). Vary which long runs are quality vs. purely easy based on the week's phase and how the athlete's been recovering (last3WeeksSummary) — don't make every long run a quality one, and never make a base-phase or recovery-week long run anything but easy.
    ...
    - "hill_repeats": sets MUST be non-empty, e.g. 24x35s uphill → durationSec=35, restBetweenReps "jog back down". This is hard-effort work (rule 6/workout_mechanics_and_first_principles.hill_intervals.intensity) — RPE 7-9/10, not a controlled sub-threshold session, and never titled/grouped as T1 or T2.
...
11. VOICE — title and focus read as this one athlete's personal season, built by one coach for them specifically (e.g. a focus string can say "I'm building your base here" in first person singular), never plural "we"/"our" or generic third-person "the program"/"the team" framing.
