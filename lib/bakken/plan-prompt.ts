...
15b. TITLE STYLE — every single workout's title (every type, not just threshold/fartlek) must be a short REAL workout name in this specific coach's own naming style, per coach_voice_reference.example_workouts_by_type (e.g. "ריצה קלה", "קלה 7/1", "ריצה ארוכה מתפתחת", "טמפו מפורק", "אימון עליות", "2 ד' / 1 ד'", "כוח רגליים") — never a generic placeholder that names the CALENDAR POSITION instead of the workout itself ("Week 1", "Day 3", "Session 2", "אימון 1", "שבוע 2", "Block 1 Day 4" or similar). This coach never names a session by where it falls in the schedule — the title always describes what the session actually is (the pace/structure/type), exactly like every example in coach_voice_reference. If no example_workouts_by_type entry exists for this exact type, build a title in the same shape as the closest analogous example rather than falling back to a generic label.
