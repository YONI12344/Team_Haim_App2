...
11. DOUBLE THRESHOLD DAYS (methodology_foundations.the_breakthrough_framing + training_tiers_by_level.elite_full_norwegian_week_almgren_model.tuesday_double_t/thursday_double_t) — ONLY when the athlete is on the elite_full_norwegian_week_almgren_model track (daysPerWeek >= 6 and experienceLevel advanced/professional, or the current stage explicitly calls for it): pick at most 2 non-consecutive days per week (e.g. Tue/Thu, per the elite week template) and emit TWO separate workout objects that share the exact same "date" — one with session "am" (longer repeats, LT1-focused, lactate ~1.8-2.2 anchored on "Lactate ~2.0") and one with session "pm" (shorter repeats, LT2-focused, lactate ~2.8-3.2 anchored on "Lactate ~3.0"), per core_coaching_mechanics.tone_management_system.daily_structure (AM = longer repeats to moderate tone, PM = shorter faster work on the primed system). VARY THE REP FORMAT — don't default to the same pairing every single double-threshold day across the season; rotate through workout_library.signature_threshold_intervals (5x6min, 25x400m, 12x1000m, 4x10min) as the core named-session pool, extending to close analogous formats when it fits the phase (2km/3km/mile reps for AM, 1000m/800m/400m for PM — the same "apply by extension of the given principles" logic as rule 2 in the skeleton prompt) so back-to-back double-threshold sessions feel genuinely different, not copy-pasted. PROGRESSION ACROSS THE SEASON: base phase favors the longer end of both AM and PM ranges (AM: 3km/2km/8-10min reps; PM: 1000m/mile reps) at controlled sub-threshold pace, building durability. Build/peak sharpens BOTH sessions shorter and faster toward race-pace-adjacent effort — AM moves toward mile/1km reps, PM moves further down to 800m/400m reps, sharper each — this is coaching application of the brain's own tone/intensity progression, not a separate zone. EXCEPTION: for a marathon-distance athlete, don't over-shorten either session even at peak — marathon pace sits close to LT2 (per distance_specific_adjustments.marathon.strategy, MP work belongs in long runs too), so keep some longer/marathon-pace-adjacent reps in the mix throughout on both AM and PM, rather than converging on short fast reps the way a 5k/10k athlete's peak work would. On a special/lighter double-threshold day (not every week — this is a variety option, not the default pairing), the AM session can instead be the coach's signature Kenyan Fartlek (<coach_voice_reference>.signature_kenyan_fartlek) with the PM session as a fast ladder — e.g. 200m x10 and/or 300m x8 at fast pace with ~90s medium-effort rest between reps, per signature_kenyan_fartlek.usage. Never do double-threshold for recreational or intermediate tracks — they get exactly one workout object per date, session "other".
