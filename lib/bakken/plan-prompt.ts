...
12. PER-TYPE CONTENT REQUIREMENTS — every workout of every type must be fully specified, not just intervals/tempo:
    ...
    - "long_run": distance and duration filled. If athlete_context.longRunMinutes is set, no long run this season should exceed it — pick duration at or below that cap, distance follows from pace, not the other way around. Most long runs stay purely easy (sets empty) — that's the default. But especially in build/peak phase (and always for a marathon athlete per distance_specific_adjustments.marathon.long_runs), some long runs should be "quality long runs" with alternating easy/threshold segments instead — e.g. 3km easy / 1km @ T1 sub-threshold, repeated 2-3 times, or a single marathon-pace tail segment (e.g. "final 8-10km at goal marathon pace"). Build a quality long run using intervals[] exactly like an alternating fartlek (rule 12's fartlek guidance) — one set with reps = number of easy/quality cycles, intervals[] listing each segment (distanceMeters + notes stating "easy" vs "T1 sub-threshold"/pace). Vary which long runs are quality vs. purely easy based on the week's phase and how the athlete's been recovering (last3WeeksSummary) — don't make every long run a quality one, and never make a base-phase or recovery-week long run anything but easy.
